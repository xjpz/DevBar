...
<p align="center">
  <strong>macOS 菜单栏工具，实时监控 GLM 与 OpenAI 用量</strong><br>
  <a href="#安装">安装</a> · <a href="#功能特性">功能</a> · <a href="#预览">预览</a> · <a href="#开发">开发</a>
</p>
...
- **多 Provider 支持** — 在同一个菜单栏应用中管理 GLM 与 OpenAI，并可配置显示优先级
- **菜单栏实时显示** — 在 macOS 菜单栏直接显示 GLM 最高用量百分比
- **Token 用量监控** — 实时显示 Token 和时间的使用进度，颜色动态变化
- **MCP 使用明细** — 按模型分项展示 MCP 调用次数（search-prime、web-reader 等）
- **订阅状态管理** — 自动检测套餐有效性，显示订阅到期时间与续费价格
- **按 Provider 登录** — 登录页可直接切换 Provider，支持 GLM 浏览器/API Key 登录，以及 OpenAI Access Token 登录
- **账号与优先级设置** — 支持按 Provider 启用/停用、调整显示顺序、分别编辑凭证
- **本地通知** — 低额度提醒、用尽提醒、额度重置提醒，智能防抖
- **自动刷新** — 支持 3/5/10/30 分钟定时刷新或手动刷新，关闭窗口自动暂停
- **设置页分组** — Tab 标签分组：通用、通知、账号、关于
...
1. **登录**
   - **选择 Provider** — 在登录页切换 `GLM` 或 `OpenAI`
   - **GLM** — 支持浏览器登录或 API Key 登录
   - **OpenAI** — 支持粘贴 Access Token，或从 `~/.codex/auth.json` 读取 `access_token`
2. **查看用量** — 点击菜单栏图标展开面板，查看 Token / MCP 用量
3. **手动刷新** — 点击刷新按钮立即获取最新数据
4. **设置** — 点击齿轮图标打开设置面板
   - **通用** — 切换菜单栏图标、调整刷新间隔、登录时启动、Dock 栏显示
   - **通知** — 启用低额度/用尽/重置提醒，设置低额度阈值
   - **账号** — 管理启用的 Provider、调整显示优先级、编辑 GLM / OpenAI 凭证
...
│   ├── QuotaProvider.swift        # Provider 定义与账号配置模型
...
│   ├── OpenAIAPIClient.swift     # OpenAI 用量 API 客户端
│   ├── AuthService.swift         # 认证状态管理
│   ├── CodexAuthFileLoader.swift # 读取 ~/.codex/auth.json 中的 OpenAI access_token
...
│   ├── OpenAIQuotaViewModel.swift # OpenAI 用量数据与刷新逻辑
│   └── UpdateViewModel.swift    # 自动更新检查
├── Views/
│   ├── MenuBarView.swift        # 支持 Provider 切换的主弹出面板
│   ├── LoginView.swift          # 支持 Provider 切换的登录页
│   ├── QuotaRowView.swift       # 单项用量进度条
│   ├── SettingsView.swift       # 设置面板主容器
│   ├── SettingsGeneral.swift    # 通用设置页
│   ├── SettingsAccounts.swift   # Provider 凭证与优先级设置
...
认证方式：
- `GLM` — `Authorization` 请求头 + `bigmodel_token_production` Cookie，或 API Key
- `OpenAI` — Bearer `Access Token`，可选 `ChatGPT-Account-Id`
